-- Phase 3 step 2 prerequisite: orders.slip_path.
--
-- place_order previously buried the transfer slip's storage path inside
-- the free-text `notes` field ("Amount paid: X | Slip: <path>"), which is
-- fragile for the admin app to parse reliably. A dedicated column is a
-- direct, robust lookup instead.

ALTER TABLE "public"."orders" ADD COLUMN "slip_path" text;
...
BEGIN
  ...
  -- slip_path is now its own column (see above) — notes just carries the
  -- human-readable summary, no longer needs to embed the path itself.
