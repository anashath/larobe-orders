-- Phase 3: reconcile place_order's statuses with the admin app's existing
-- fulfillment pipeline, instead of a parallel one.
--
-- Two real bugs fixed here, found while planning the admin app's web-order
-- UI (not yet triggered since nothing used these orders in the admin app
-- yet):
--
-- 1. place_order never set stock_deducted, even though it deducts stock
--    immediately for cod/transfer orders (everything except pre-order).
--    The admin app's updateOrderStatus() uses that flag to decide whether
--    to deduct-again or restore stock on status changes — with it
--    unset (false), moving a web order through the pipeline would have
--    double-deducted stock, and cancelling one wouldn't have restored it.
--
-- 2. cod orders got a bespoke status ('New') the admin app's pipeline
--    doesn't recognize at all — no badge, no action buttons, stuck.
--    Changed to 'pending', the exact status in-store orders start at, so
--    cod web orders join the *existing*, already-correct pipeline with no
--    new admin-app logic needed beyond a source badge/filter. transfer and
--    preorder keep their distinct statuses, since "awaiting payment proof"
--    and "awaiting stock allocation" are genuinely new concepts the old
--    vocabulary has no equivalent for — those get their own small, explicit
--    UI in this phase (Verify/Reject, pre-order queue) rather than being
--    forced into the existing pipeline.
...
BEGIN
  ...
  IF p_payment_method IN ('cod', 'transfer') THEN
    SELECT count(*) INTO v_recent_count
      ...
      WHERE tenant_id = v_tenant_id
        ...
        AND status IN ('pending', 'Payment pending verification')
  ...
  -- cod joins the admin app's existing pipeline at its normal starting
  -- point. transfer/preorder keep distinct statuses the old vocabulary
  -- has no equivalent for.
  v_status := CASE p_payment_method
    WHEN 'cod' THEN 'pending'
  ...
  -- stock_deducted must match reality: true for cod/transfer (stock is
  -- reserved immediately, below), false for preorder (deducted later on
  -- allocation). The admin app's updateOrderStatus() reads this flag to
  -- decide whether to deduct again or restore on future status changes —
  -- getting it wrong here caused bug #1 above.
  INSERT INTO public.orders (
    tenant_id, num, customer_name, contact, product, price, status, source,
    order_date, order_time, payment, payment_done, notes, delivery, slip_path,
    stock_deducted
  ) VALUES (
    ...
    v_payment_note, p_customer_address, p_slip_path,
    NOT v_is_preorder
  )
