-- Phase 2 step 4: place_order. Atomic, server-side-priced guest checkout.
--
-- SECURITY DEFINER, callable only via the service role (the place_order
-- Edge Function) — never directly from the browser. Prices, promotions,
-- and stock are always looked up from the database here, never trusted
-- from the caller (HANDOFF.md rule 5) — callers only ever send
-- product_id + quantity. Pre-order items skip stock deduction (deducted
-- later on allocation, per HANDOFF.md §6 — allocation flow is Phase 3
